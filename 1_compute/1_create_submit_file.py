...
in_paths = [
    "/data/project/connectme/EEGs/edf/resting",
    "/data/project/connectme/EEGs/edf/stim",
]

in_paths = [Path(x) for x in in_paths]

out_path = "/data/project/connectme/EEGs/results"
...
exec_string = ("1_compute_markers.py --path $(dbpath) --opath $(opath) "
               "--subject $(subject)")
...
with open(submit_fname, "w") as submit_file:
    ...
    for t_path in in_paths:
        ...
        for t_subject in subjects:
            ...
            submit_file.write(f"opath={out_path}/{kind}\n")
